...
// Adicione a nova categoria "aniversario" à lista existente
const COMMAND_CATEGORIES = ['info', 'utility', 'download', 'sticker', 'misc', 'aniversario', 'group', 'admin'];
...
export function getCommands(prefix) {
    const commands = [
        ...
        ...Object.keys(aniversarioCommands), // Adicione esta linha
...
export function getCommandsByCategory(prefix, category) {
    switch (category) {
        ...
        case 'aniversario':
            return Object.keys(aniversarioCommands).map(command => prefix + command); // Adicione esta linha
        case 'group':
...
export function getCommandCategory(prefix, command) {
    ...
    for (let category of categories) {
        if (getCommandsByCategory(prefix, category).includes(command)) {
            ...
            break; // Adicionado um 'break' para otimizar o loop
        }
    }
...
export function getCommandGuide(prefix, command) {
    ...
    switch (commandCategory) {
        ...
        case 'misc':
            ...
            break;
        case 'aniversario':
            const aniversario = aniversarioCommands;
            guide_text = guide_header_text + aniversario[removePrefix(prefix, command)].guide;
            break;
        case 'group':
